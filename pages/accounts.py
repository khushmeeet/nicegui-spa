    ...
    with ui.column().classes("w-full h-full text-lg"):
        ...
        if right_drawer and right_drawer_rendered_by != "accounts":
            ...
            with right_drawer:
                ...
                ui.slider(
                    min=300, max=800, value=state["width"], step=10,
                    on_change=lambda e: on_width_change(e.value)
                ).classes("m-2").props("color=primary").bind_value(state, "width")
                ui.input(
                    label="Right Drawer Account Text",
                    value="",
                ).classes("m-2").props("color=primary").bind_value(state, "text")
