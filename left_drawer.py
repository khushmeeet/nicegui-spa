...
def menu_item(
    ...
    with ui.item().props("clickable ripple v-ripple").classes("p-3 pl-7 mb-2 text-grey-14").on("click", lambda: open_page(key, action, router)) as item:
...
def left_drawer(router: router.Router = None):
    with ui.left_drawer(top_corner=True, bottom_corner=True).props("bordered").classes("p-0 gap-3 bg-gray-100") as left_drawer:
        with ui.element("div").classes("p-0 w-full nicegui-header mt-3").style("height: 68px;"):
            with ui.item().props("clickable").classes("p-5 text-3xl w-full h-full").on("click", lambda: open_dashboard(router)):
                with ui.item_section().props("avatar").style("min-width: 0;"):
        ...
        with ui.list().classes("text-xl").classes("w-full justify-start items-start"):
            for key, data in pages.items():
                pages[key]["object"] = menu_item(key, data["icon"], data["label"], data["show"], left_drawer, router)
        return left_drawer
